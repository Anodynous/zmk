...
static void press_and_release_combo(const struct behavior_macro_config *cfg,
...
static void press_combo(const struct behavior_macro_config *cfg,
                        struct zmk_behavior_binding_event event) {
    for (int index = 0; index < cfg->behavior_count; index++) {
        const struct device *behavior = device_get_binding(cfg->behaviors[index].behavior_dev);
        if (!behavior) {
...
static void release_combo(const struct behavior_macro_config *cfg,
                          struct zmk_behavior_binding_event event) {
    for (int index = 0; index < cfg->behavior_count; index++) {
        const struct device *behavior = device_get_binding(cfg->behaviors[index].behavior_dev);
        if (!behavior) {
...
static int on_keymap_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    if (cfg->mode == MODE_KEY_DOWN) {
        press_and_release_combo(cfg, event);
    } else if (cfg->mode == MODE_HOLD) {
        press_combo(cfg, event);
    }
...
static int on_keymap_binding_released(struct zmk_behavior_binding *binding,
    ...
    const struct device *dev = device_get_binding(binding->behavior_dev);
    const struct behavior_macro_config *cfg = dev->config;
    if (cfg->mode == MODE_KEY_UP) {
        press_and_release_combo(cfg, event);
    } else if (cfg->mode == MODE_HOLD) {
        release_combo(cfg, event);
    }
    return ZMK_BEHAVIOR_OPAQUE;
}
